namespace ADJInsc.Core.Logica
{
    ...
    public class LogicHelper
    {
                ...
                {                  
                    ...
                    using (SqlCommand cmdInsDomicilio = new SqlCommand(queryDomicilio, con))
                    {
                        ...
                        foreach (var item in inscViewModel.GrupoFamiliar)
                        {
                            ...
                            cmdInsGrupo.Parameters.Add(new SqlParameter("@insf_nombre", item.InsfNombre.Trim()));
        ...
        public async Task<ResponseViewModel> PostServerModelo(ModeloCarga model)
        {
           ...
           // var queryF = "SELECT ins_id FROM InsFamilia where insf_numdoc = @dni";
            ...
            using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
            {
                try
                {
                    string query = "SELECT Id FROM Usuario where NombreUsuario = @nombreUsuario and usu_estado = 'A' ";
                    using (SqlCommand cmdConsulta = new SqlCommand(query, con))
                    {
                        //  *************************     1_ Consultar si existe el email y ademas si esta habilitado (A)
                        cmdConsulta.CommandType = CommandType.Text;
                        cmdConsulta.Parameters.Add(new SqlParameter("@nombreUsuario", model.usuario.Trim()));
                        ...
                        using (SqlCommand cmdSegundaConsulta = new SqlCommand(query, con))
                        {
                            ...
                            cmdSegundaConsulta.Parameters.Add(new SqlParameter("@dni", model.dni.Trim()));
                            if (con.State == ConnectionState.Closed)
                                await con.OpenAsync();
                            ...
                            if (objetoId != null)
                            {
                                ...
                                insertTitular = "UPDATE Inscriptos SET " +
                                                            ...
                                                            " ins_email = @ins_email, IdUsuario= @IdUsuario,  ins_estado = 'A', IdDomicilio = @idDomicilio, " +   //aqui saque el estado ins_estado = 'A',
                            ...
                            {
                                insertTitular = "INSERT INTO Inscriptos (" +
                                             "  ins_tipflia, IdTipoFamilia,  ins_nombre, ins_numdoc, ins_estado, ins_email, IdUsuario, cuit_cuil, cuit_cuil_uno, cuit_cuil_dos, ins_telef )" +
                                             "VALUES " +
                                              "( @ins_tipflia, @IdTipoFamilia, @ins_nombre,  @ins_numdoc, 'A' , " +
                                              "  @ins_email, @IdUsuario, @cuit, @cuitUno, @cuitDos, @telefono ); SELECT SCOPE_IDENTITY();";
                            ...
                            using (SqlCommand cmd = new SqlCommand(insertUsuario, con))
                            {
                                ...
                                cmd.Parameters.Add(new SqlParameter("@NombreUsuario", model.usuario.Trim()));
                                cmd.Parameters.Add(new SqlParameter("@ClaveUsuario", model.clave.Trim()));
                                ...
                                using (SqlCommand cmd1 = new SqlCommand(insertTitular, con))
                                {
                                    ...
                                    cmd1.Parameters.Add(new SqlParameter("@ins_numdoc", model.dni.Trim()));
                                    cmd1.Parameters.Add(new SqlParameter("@ins_email", model.email.Trim()));
                                    ...
                                    cmd1.Parameters.Add(new SqlParameter("@idDomicilio", model.IdDomicilio));
                                    if (con.State == ConnectionState.Closed)
                                        await con.OpenAsync();
                                    ...
                                    {
                                        cmdDelete.Parameters.Add(new SqlParameter("@numDoc", model.dni.Trim()));
        ...
        private async Task<DataTable> GetDTDomicilio(int idDomicilio, int idInscripto)
        {
            try
            {
                Connection();
                ...
                else
                {
                    if (idDomicilio == 0)
                    {
                        ...
                        cmdD.Parameters.Add(new SqlParameter("@insdId", idInscripto));
        ...
        public async Task<UsuarioTitularViewModel> GetInscriptoGrupo(int dni)
        {
            ...
            if (grupoFamiliarExiste.Count > 0)
            {
                ...
                dt = await GetDTInscripto(0,idTitular);     //aqui lleno si se obtiene primero el grupo familiar
                //pList.GrupoFamiliar = grupoFamiliarExiste;
            }
            else
            {
                dt = await GetDTInscripto(dni,0 );          //aqui lo obtengo con el dni
            }
            ...
            if (idDomicilio > 0)
            {
                ...
                var dt1 = await GetDTDomicilio(idDomicilio, 0);
            ...
            {
                var dt1 = await GetDTDomicilio(0, idTitular);
            ...
            var dt2 = await GetDTSituacionLaboral(idTitular);
            ...
            pList.GrupoFamiliar = await GetGrupoFamiliar(idTitular, 0);
        ...
        public async Task<UsuarioTitularViewModel> GetInscripto(int dni)
        {
            ...
            {                
                ...
                if (insId > 0)
                {
                    ...
                    {
                        var dt1 = await GetDTDomicilio(0, insId);
        ...
        public async Task<string> Recuperar(string email, int dni)
        {
            ...
            try
            {
                var query = "select IdUsuario , ins_email , ins_estado from Inscriptos where ins_numdoc=@dni";
                ...
                if (dt.Rows.Count > 0)
                {
                    var inscEmail = string.Empty;
                    var estado = string.Empty;
                    var idUsuario = 0;
                    foreach (DataRow row in dt.Rows)
                    {
                       estado = (string)row["ins_estado"];
                       inscEmail = (string)row["ins_email"];
                    ...
                    var claveUsuario = await cmd1.ExecuteScalarAsync();

                    if (string.IsNullOrEmpty((string)claveUsuario)) return null;
        ...
        public async Task<InscViewModel> GetLogin(string usuario, string clave)
        {
            ...
            try
            {
               // 1_ obtener datos del titular junto al usuario y clave
                        query =
                            "SELECT ins_id, ins_ficha ,ins_tipflia ,ins_fecins, ins_nombre, ins_tipdoc, IdTipoFamilia, ins_discapacitado, ins_minero, ins_veterano " +
                                   " ,ins_numdoc ,ins_email ,ins_telef ,ins_estado ,ins_fecalt, cuit_cuil, cuit_cuil_uno, cuit_cuil_dos, IdDomicilio, U.ClaveUsuario  FROM Inscriptos as I " +
                            " INNER JOIN Usuario as U on I.IdUsuario = U.Id " +
                            " WHERE U.NombreUsuario = @usuario  and U.usu_estado = 'A' and ( I.ins_estado = 'E' or I.ins_estado = 'I' or I.ins_estado = 'A' or I.ins_estado = 'S'  ) ";   //and U.ClaveUsuario = @clave
                ...
                if (dt.Rows.Count > 0 )
                {
                    var claveControl = string.Empty;
                    var insId = 0;

                    foreach (DataRow item in dt.Rows)
                    {
                        claveControl = ConvertFromReader<string>(item["ClaveUsuario"]);
                        if (claveControl.Trim() == clave.Trim())
                        {
                            pList.InsId = ConvertFromReader<int>(item["ins_id"]);
                            pList.InsEmail = ConvertFromReader<string>(item["ins_email"]);
                            pList.InsEstado = ConvertFromReader<string>(item["ins_estado"]);
                    ...
                    if (insId > 0)
                    {
                        ...
                        if (idDomicilio > 0)
                        {
                            ...
                            var dt3 = await GetDTDomicilio(idDomicilio, 0);
                        ...
                        {
                            var dt3 = await GetDTDomicilio(0, insId);
                            ...
                            if (dt3.Rows.Count > 0)
                            {
                                foreach (DataRow item in dt3.Rows)
                                {
                                    ...
                                    pList.IdDomicilio = ConvertFromReader<int>(item["insd_id"]);
                                }
                            }
                        }
